...
if (rev(split_path(homeuser))[1] != subDir) { #Need to create it
...
#create symblink file to make FinAnalytics and data eboth usable
if(file.exists("../data")) {
  print("Conflict cannot crate linked called ../data: file exists.")
} else {
  system(paste0("ln -s ",file.path(homeuser, subDir)," ../data"))
}

#obtain entire market from start to end dates: daily adjClose
dir <- "MVO3.2020.11" #EDIT HERE
res <- findAllPrices(dir = dir,
                     start = "2017-11-28", #EDIT HERE
                     end   = "2020-11-29",needToAcquire=T) #EDIT HERE
